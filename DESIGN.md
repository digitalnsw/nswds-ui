...
- **On a band:** on a solid band every segment that is not itself solid takes the band's label colour as its ink, so a defaulted or soft segment is white on blue, never blue on blue. On a soft or surface band a segment sits on the tint, so its own colour's ink steps as a lone soft Button's does: a tertiary segment on a tertiary tint is `-700` on the `-700` tint, never `-600` on it, and a danger segment on that tint keeps danger's ink — the group's colour is a default, never an override.
