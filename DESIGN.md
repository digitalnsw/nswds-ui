...
Buttons draw _optical_ borders: the border width is a variable (1px, or 2px for outline/surface variants) subtracted from the padding, so every variant occupies an identical outer box, and the inner radius is `calc(var(--radius-sm) - 1px)` so the two curves stay concentric.
...
- **Tints:** `soft` and `surface` paint the ink over a 10% / 5% tint of itself, and the ink on a tint must clear 4.5:1 on that tint. Tertiary, accent, success and warning sit at 4.5–5.2:1 on pure white and drop under the floor on their own tints, so on a tint their ink steps to `-700` (not `-800`, which is primary's own ink) — and because it is the ink that steps, the tint, the overlays and the ring step with it. Never a paler tint instead: an ink at 4.5:1 on white has no tint of itself to spare. The measured pairs live in `styles.tintInk` in button.tsx.
...
- **Emphasis:** a segment that names `solid` or `soft` keeps it — a solid Save beside a ghost Cancel is the row's primary action; two solid segments (a split button) divide with a hairline in the label colour at 30%. `outline`, `ghost` and `link` children render as plain segments, so the shadcn idiom of writing `variant='outline'` on every child draws no second border. `surface` is not a segment emphasis: its 2px border would sit inside the group's frame.
- **On a band:** every segment that is not itself solid takes the band's label colour as its ink and gives up any tint of its own, so a defaulted or soft segment on a solid band is white on blue, never blue on blue and never white on a paler patch of the band — a soft segment's fill re-inks to white like everything else, and two 10% layers of it are enough to take the label under 4.5:1 on the bands that sit near the floor. On a soft or surface band a segment sits on the tint, so its own colour's ink steps as a lone soft Button's does: a tertiary segment on a tertiary tint is `-700` on the `-700` tint, never `-600` on it, and a danger segment on that tint keeps danger's ink — the group's colour is a default, never an override.
- **Geometry:** the frame is an inset ring, so a group is exactly as tall as the lone Button beside it. Corners are clipped by the group, which is why a segment's focus ring sits 2px inside its edge rather than 2px onto the page — the same inversion the nav rails make inside a scroll container — and a solid segment rings in its label colour, since in light mode its ink and its fill are one token. The text cell is 16px semibold on the sunken surface, never fine print. The boundary before a disabled segment is drawn on the segment before it, so it does not fade with the disabled one; between two adjacent disabled segments the shared boundary has no enabled edge to move to and fades with the pair, which is the right reading inside an unavailable run.
- **Scope:** Buttons only, as direct children — an input with an attached action is InputGroup's job, groups do not nest, and a segment wrapped in another element keeps its label and band treatment but loses the hairlines it shares with its neighbours, because hairlines are drawn on the segments' shared edges. `size='icon'` on a segment renders as `iconOnly` at the group's own step, since the group clips its box and would cut off the square's 44px touch expansion, and a square pinned to a different step could not stretch back to match its neighbours. React context follows a portal, so every popup in the package resets the group at its portal (`ButtonGroupBoundary`); an overlay from another library opened from a segment should do the same.
