    ...
    docs: {
      description: {
        story: wcagStoryMeta({
          ...
          caveat:
            'Low-contrast colours (white, secondary) are rendered on a grey-800 surface, which is where they are meant to be used. On a solid band every non-solid segment takes the label colour as its ink and gives up its own tint, so a soft segment on a band reads as the ghost segments beside it rather than blue on blue: its two 10% fill layers, re-inked to white, lifted the band under the label to 3.37:1 on success and 3.41:1 on tertiary, against 4.53:1 and 4.57:1 once they are gone. On a soft or surface band each segment steps its own colour ink as a lone soft Button does, so for tertiary, accent, success and warning the band and its segments read in the -700 step, which clears the floor where the -600 ink did not (the measured pairs are in styles.tintInk in button.tsx). The play asserts that step; axe measures its result.',
        }),
      },
    },
  },
