...
function resolveGroupDefaults(
  ...
  if (!group) {
    return { variant: variant ?? 'solid', color, size, segment: undefined, onTint: undefined }
  }
  ...
  return {
    ...
    // danger segment on a tertiary band stays danger.
    onTint: resolved !== 'solid' && TINT_VARIANTS.has(group.variant),
  }
}

/**
 * `resolveGroupDefaults` against the enclosing ButtonGroup, if any. The
 * context types `color`, `size` and `variant` as strings so its module can
 * ship with every popup (see button-group-context.tsx); only ButtonGroup
 * provides it, from props typed with Button's own unions, so the narrowing
 * here is sound.
 */
...
function Button({
  ...
  const { variant, color, size, segment, onTint } = useSegmentDefaults({
    variant: variantProp,
    color: colorProp,
    size: sizeProp,
  })
