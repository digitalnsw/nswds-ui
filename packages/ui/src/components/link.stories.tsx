...
const meta = {
  ...
  argTypes: {
    ...
    target: {
      ...
      description:
        'Browsing context for the link target (use _blank for external).',
      table: { category: 'Behavior' },
    },
    rel: {
      control: 'text',
      description:
        'Relationship of the linked resource (e.g. noopener noreferrer).',
      table: { category: 'Behavior' },
    },
    'aria-label': {
      control: 'text',
      description:
        'Accessible name when visible text is not sufficient (e.g. icon-only).',
...
export const Default: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    if (text !== String(args.children)) {
      throw new Error(
        `Expected link text "${String(args.children)}", received "${text}".`
      )
    }
  },
}
